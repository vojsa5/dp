...
import microc.ast.{AndAnd, ArrayAccess, BinaryOp, BinaryOperator, CodeLoc, Expr, GreaterEqual, Identifier, IdentifierDecl, Not, Number, OrOr, Plus, Program, WhileStmt}
...
case class Vertex(path: Path, condition: Expr, change: mutable.HashMap[Expr, Expr => (Expr => Expr)], iterationsVal: SymbolicVal)
...
//      }
      ...
      for (edge <- pda.edges(currPath.path)) {
        ...
        var newState = symbolicState.deepCopy()
        val condition = Utility.simplifyArithExpr(BinaryOp(AndAnd, traceCondition, pda.solver.applyTheState(edge.condition, symbolicState), edge.condition.loc))
          ...
          case Status.SATISFIABLE =>
            ...
            for (update <- edge.change) {
              if (newUpdatedVariables.contains(update._1)) {
                ...
                update._1 match {
                  case Identifier(name, _) =>
                    newState = newState.addedVar(name,
                      SymbolicExpr(update._2.apply(symbolicState.getSymbolicVal(name, CodeLoc(0, 0), true).asInstanceOf[Symbolic]), CodeLoc(0, 0)))
